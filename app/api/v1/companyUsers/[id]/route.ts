...
export const PUT = async (req: NextRequest, { params }: { params: { id: string } }) => {
    ...
    try {
        ...
        if (!existingItem) {
            return NextResponse.json(
                { message: 'This company user does not exist' },
                { status: 404 }
            );
        }
        ...
        if (!companyUserUserId && !companyUserCompanyId) {
            return NextResponse.json(
                { message: 'At least one field is required to update company user' },
                { status: 400 }
            );
        }
        ...
        updateData.companyUserUserId = companyUserUserId || existingItem.companyUserUserId;
        updateData.companyUserCompanyId = companyUserCompanyId || existingItem.companyUserCompanyId;

        // Mettre à jour l'item dans la base de données
        const updatedCompanyUser = await prisma[collection].update({
            where: { [collectionId]: id },
            data: updateData,
        });

        // Retourner l'item mis à jour avec un statut 200 (OK)
        return NextResponse.json(updatedCompanyUser, { status: 200 });
    }
    catch (error) {
        // En cas d'erreur, renvoyer un statut 500 (Internal Server Error)
        return NextResponse.json(
            { message: 'Cound not update the company user', error },
            { status: 500 }
        );
    }
}
