...
export const PUT = async (req: NextRequest, { params }: { params: { id: string } }) => {
    ...
    try {
        ...
        const { cityName, cityCountryId, cityGMTOffset } = await req.json();
        ...
        if (!cityName && !cityCountryId && !cityGMTOffset) {
            return NextResponse.json(
                { message: 'At least one field is required to update city' },
                { status: 400 }
            );
        }
        ...
        updateData.cityName = cityName || existingItem.cityName;
        updateData.cityCountryId = cityCountryId || existingItem.cityCountryId;
        updateData.cityGMTOffset = cityGMTOffset || existingItem.cityGMTOffset;

        // Mettre à jour l'item dans la base de données
        const updatedCity = await prisma[collection].update({
            where: { [collectionId]: id },
            data: updateData,
        });
        ...
        return NextResponse.json(
            { message: 'Cound not update the city', error },
            { status: 500 }
        );
    }
}
